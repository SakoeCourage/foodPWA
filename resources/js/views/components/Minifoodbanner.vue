<template>
    <div>
        <nav class=" ">
          <nav class="flex items-center justify-between mb-2 ">
            <span class=" flex items-center gap-2">
              <span class="h-6 w-6 aspect-square ">
                <img class="object-cover rounded-full w-full h-full" :src="setImage(created_at,thumbnail)" alt="">
              </span>
         ...
         <nav class=" aspect-square overflow-hidden  rounded-md block relative">
              ...
              <nav class="absolute inset-0  bg-gray-400/10 z-10">
                <nav class="w-full h-full relative">
                  ...
                  <nav class="absolute bottom-0 h-[30%] w-full bg-gradient-to-b from-gray-400/10 to-gray-400/50 flex items-end pl-5 pb-5 ">
                    <router-link :to="'/viewdish/'+slug" class=" text-gray-50  font-semibold">{{ dish_name }} </router-link>
                  </nav>
                </nav>
              </nav>
            </router-link>
